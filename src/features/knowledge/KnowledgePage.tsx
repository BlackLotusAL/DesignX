...
export function KnowledgePage({
  ...
  const { knowledgePackages } = useAppState();
  ...
  return (
    <div className="page page--knowledge">
      <header className="page-header">
        ...
        <Button onClick={() => onImportOpenChange(true)} variant="primary">
      ...
      <section aria-label="知识包列表" className="data-table knowledge-table">
        ...
        {filteredPackages.map((knowledgePackage) => (
          ...
          >
            ...
            <span>{knowledgePackage.updatedAt}</span>
            <span className="row-actions">
              <button
                aria-label={`更多 ${knowledgePackage.name}`}
        ...
        {filteredPackages.length === 0 ? (
          <div className="empty-state">
            ...
            <Button onClick={() => onImportOpenChange(true)}>导入文档</Button>
          </div>
        ) : null}
      </section>
