...
import type { AnalysisRange, NavigationId } from '../../types';
...
export function AnalysisComposer({ onNavigate }: AnalysisComposerProps) {
  ...
  const { startAnalysis } = useAppActions();
  const { repositories, knowledgePackages, tasks, settings } = snapshot;
  ...
  const [range, setRange] = useState<AnalysisRange>('baseline');
  ...
  useEffect(() => {
    const availableIds = new Set(
      publishedKnowledge.map((item) => `${item.id}@${item.version}`),
    );
    if (!availableIds.has(primaryKnowledgeId)) {
      setPrimaryKnowledgeId(
        publishedKnowledge[0]
          ? `${publishedKnowledge[0].id}@${publishedKnowledge[0].version}`
          : '',
      );
    }
    if (secondaryKnowledgeId && !availableIds.has(secondaryKnowledgeId)) {
      setSecondaryKnowledgeId('');
    }
  }, [primaryKnowledgeId, publishedKnowledge, secondaryKnowledgeId]);
  ...
  const submit = async () => {
    ...
    const taskId = await startAnalysis({
      ...
      knowledgeVersionIds: knowledgeIds,
      range,
      focus,
    });
    if (!taskId) return;
      ...
      <div className="analysis-composer__controls">
        ...
        <label className="composer-control composer-control--wide">
          ...
          <Select
            aria-label="分析范围"
            onChange={(event) =>
              setRange(event.target.value as AnalysisRange)
            }
            value={range}
          >
            <option value="baseline">上次成功分析 → 当前提交</option>
            <option value="current">当前代码基线</option>
            <option value="recent10">最近 10 个提交</option>
          </Select>
        </label>
